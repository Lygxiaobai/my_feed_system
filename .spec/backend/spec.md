...
The child nodes own the detailed account, feed, video, interaction, social, wallet, notification, dm, analytics, ops, report, danmaku, history, outbox, message-consumer, and runtime contracts. This parent spec owns only the process boundary and the rules that connect those nodes.
