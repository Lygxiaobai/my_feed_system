...
The unsigned account surface leads with email and a six-digit code. Sending a code starts a short cooldown on the button. WeChat and QQ controls are visible but only explain that they are not available yet. Password login lives on a dedicated route so it is not mixed into the email form. The dedicated register route sends the visitor back to this email flow instead of collecting a separate username and password. The signed-in hub also reaches the wallet, daily check-in, lottery pages, and browsing history. History sits with those hub actions and as a metric on the same row as works, then lists unfinished and completed videos. That surface is owned by `history-ui`. The desktop top bar reaches the same wallet and publish pages, the account hub, the notifications inbox, and frontend-only placeholders for a client download, wallpapers, and messages. The recharge control is labeled 充积分. Those placeholders do not call a backend. Notifications are owned by `notification-ui`. An account bound to a digits-only test-domain email also sees an operations entry on the signed-in hub; that surface is owned by the ops-ui spec.
