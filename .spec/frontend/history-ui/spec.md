...
账号页的浏览历史入口和签到、钱包放在同一排，数据栏里也有一格「浏览历史」，列表顶部再和作品、点赞并列切换。未看完卡片显示封面进度条和已看到的时刻；已看完不显示进度条。点进详情后，只有服务端（或本机兜底）给出可恢复进度时才 seek，并且等媒体元数据就绪。信息流只采集进度，不按历史 seek。移除一行只删该用户自己的历史，不影响视频本身。
