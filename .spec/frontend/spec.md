...
## raw source
The frontend is a Vue application with a single bootstrap entry, client-side routing, Pinia stores, API modules, and behavior-facing views for feed, video, account, wallet, check-in, lottery, and interactions.
...
`components/AppShell.vue` owns the navigation surface. Primary destinations stay on the desktop sidebar and the compact bottom navigation; there is no overflow menu duplicating those destinations. The desktop top bar, aligned with search, also hosts utility entries: recharge, a client download placeholder, a wallpaper placeholder, notifications, messages, publish, and the account avatar. Entries that already have an API open that page; the rest open a frontend-only placeholder that does not invent a backend. Signed-out visitors who hit an authenticated utility are sent to the account page. The compact "mine" destination still resolves to the account hub. The chrome shows no routing or diagnostic identifiers.

The child nodes own the detailed feed, video, account, wallet, interaction, and analytics contracts. This parent spec owns application startup, routing, cross-view state boundaries, and the rule that capability behavior remains in its owning child node.
