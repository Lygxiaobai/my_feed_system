...
| 流量拒绝计数 | String | `traffic:deny:<dim>:<subject>` | 1m | 短窗内被 429 的次数，用于判定异常主体。 |
| 流量处罚箱 | String | `traffic:penalty:<dim>:<subject>` | 10m | 异常主体在冷却期内使用更紧的全局天花板。 |
...
分层而不是只靠单条路由阈值：跨接口扫一遍就能绕开「只限点赞、不限推荐」这类缺口。

```
进程内并发保护 → 全局 IP/账号天花板 → 异常主体处罚箱 → 按路由固定窗口
```

`/ping`、`/metrics`、`/ops/gate`、支付宝异步通知和 `/static/` 不参与计数：前三者是监测，通知丢了会漏入账，静态媒体是播放本身。

### 9.1 全局天花板与处罚箱

| 规则 | 维度 | 阈值 |
| --- | --- | --- |
| 全局天花板 | IP / 账号 | 1 分钟 600 次 / 800 次 |
| 处罚箱 | IP / 账号 | 1 分钟 60 次 / 80 次，冷却 10 分钟 |
| 入箱条件 | IP / 账号 | 1 分钟内被拒绝 10 次 |
| 进程内并发 | 单进程 | 同时处理 512 个请求，超出返回 503 |

阈值写在代码默认值里，不新增 YAML，避免部署 overlay 把仓库配置盖掉后规则静默失效。
...
| 推荐混排 | IP | 1 分钟 40 次 |
| 视频读取 | IP | 1 分钟 180 次 |
| 上传 / 发布 | IP + 账号 | 10 分钟 20 次 / 12 次 |
| 点赞 / 取消点赞 | IP + 账号 | 1 分钟 60 次 / 30 次 |
| 发布评论 | IP + 账号 | 1 分钟 30 次 / 15 次 |
| 删除评论 | IP + 账号 | 1 分钟 40 次 / 20 次 |
| 关注 / 取关 | IP + 账号 | 1 分钟 40 次 / 20 次 |

设计上采用 `FailOpen`，即限流器异常时默认放行业务请求，避免 Redis 故障反向拖垮主链路。这与内容审核的取向相反：审核故障必须拦下，限流故障必须放行。

### 9.3 可观测性

1. API 暴露 `GET /metrics`，可接 Prometheus 抓取。
2. `rate_limit_total{scope,result}` 与 `traffic_penalty_applied_total{dimension}` 记录治理判定；`scope` 是规则名，不是 IP。
3. pprof 可通过配置独立开启，API / Worker 分别支持独立端口。
